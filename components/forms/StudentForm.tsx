...
import { studentSchema, StudentSchema } from '@/lib/fromValidationSchema';
...
const StudentForm = ({ type, data, relatedData, setOpen }: { type: "create" | "update"; data?: any, relatedData?: any; setOpen: Dispatch<SetStateAction<boolean>> }) => {
  const [state, formAction, loading] = useActionState(type === "create" ? createStudent : updateStudent, { success: false, error: false });
      ...
      <div className='flex justify-between flex-wrap gap-4'>
       ...
       {data && <InputField label='id' name="id" defaultValue={data?.id} register={register} error={errors?.id} hidden />}
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          >
            {classes.map((classItem: { id: number; name: number; capacity : number; _count : {students : number} }) => (
