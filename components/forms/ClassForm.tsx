...
import { classSchema, ClassSchema } from '@/lib/fromValidationSchema';
import { toast } from 'react-toastify';



const  ClassForm = ({setOpen,type,data,relatedData} : {setOpen : Dispatch<SetStateAction<boolean>>;type:"create" | "update"; data? : any,relatedData?:any}) => {
  const [state, formAction, loading] = useActionState(type === "create" ? createClass : updateClass, { success: false, error: false });
    ...
    return (
      ...
      <div className='flex justify-between flex-wrap gap-4'>
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select className="border ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("supervisorId")} defaultValue={data?.teachers}>
             {teachers.map(
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4">
          ...
          <select className="border ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full" {...register("gradeId")} defaultValue={data?.gradeId}>
             {grades.map(
